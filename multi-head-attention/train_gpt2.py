...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y= att @ v
        if self.ablated_head is not None:
            print("ABLATING", self.ablated_head)
            y[:,self.ablated_head,:,:]=0
        if self.shuffle_perm is not None:
            y=y[:,self.shuffle_perm,:,:]
