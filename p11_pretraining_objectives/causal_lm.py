...
def get_batch(tokens,B,T):
    ...
    output_tokens=tokens[ix+1:ix+B*T+1].view(B,T)
...
with open("causal_loss_2.json","w") as f:
    json.dump(losses,f)
...
with torch.no_grad():
    for _ in range(max_new_tokens):
        # Forward pass (assuming your GPT2 forward takes idx)
        # Note: adjust this if your forward function returns (logits, loss)
        logits = model(idx) 
        if isinstance(logits, tuple): 
            logits = logits[0]
            
        # Pluck the logits for the very last token
        logits = logits[:, -1, :] 
